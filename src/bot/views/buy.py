...
class BuyPremiumView(View):
    ...
    @disnake.ui.button(
        label="Премиум-версия для сервера", style=disnake.ButtonStyle.blurple
    )
    async def guild_version(self, button: Button, inter: MessageInteraction):
        if inter.guild_id is None:
            return await inter.send(
                "Чтобы приобрести подписку для сервера, необходима проводить операцию на сервере, подписку для которого вы хотите приобрести",
                ephemeral=True,
            )
    ...
    async def __buy_method(
        ...
        embed = Embed(
            title=title, color=GENERAL_COLOR, description=INSTRUCTION_BUY_TEXT
        )
