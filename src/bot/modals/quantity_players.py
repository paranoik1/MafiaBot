...
class QuantityPlayersModal(Modal):
    def __init__(self, settings: Settings):
        ...
        borders_text = (
            f"(границы {MafiaConfig.MIN_PLAYERS}-{MafiaConfig.MAX_PLAYERS_PREMIUM})"
        )
    ...
    async def callback(self, inter: ModalInteraction) -> None:
        ...
        if (
            not data["maximum_players"].isdigit()
            or not data["minimum_players"].isdigit()
        ):
        ...
        is_succes, error_message = self.settings.update_quantity_players(
            int(data["maximum_players"]), int(data["minimum_players"])
        )

        if not is_succes:
            return await inter.send(
                f"{error_message} Настройки не были сохранены..", ephemeral=True
            )
